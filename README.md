...
Раздел goal: вопрос. Ответ на вопрос - либо да, либо нет. А как? Побочным эффектом является определение значений переменных, при которых достигается ответ.
...
Переменная (именованная) - передача данных во времени и пространстве. Переменная будет конкретизированна каким-то значением. Есть механизм отказа - исправления ошибки, если система дала неправильный ответ. Служит для передачи знаний из базы знаний во внешний мир. Именованная переменная уникальна в рамках одного предложения. А вот анонимная переменная уникальна всегда. Аонимная перемеенная используется, если нас не интересует значение переменной.
...
Общая форма правила: A: - B1, B2, ..., Bn; A, Bi - термы. А - заголовок правила (фиксация (формулировка) знания о том, что между аргументами возможна истинная связь), Bi - тело (условие истинности). Это предложение. Правило - аналог теоремы. Знания содержаться в загаловке.

Факты, не содержащие переменные, называются основными. Факт - частный случай правила, потому что факт - это знания, а знания - в заголовке правила. Факт имееет загаловок, но не имеет тело (условие истинности).

Ответы отличаются значениями переменных, содержащихся в вопросе. О знаниях можно знать свойства или связь.

Подстановки и термы:

Подстановка - множество пар вида {xi = ti}, xi -переменная, ti - терм, не содержащий переменных, т.е. возможное значение переменной xi.
Подстановка O = {x1 = t1, x2 = t2, ...}. Применение подстановки к терму: AO. применение заключается в замене каждого вхождения xi на соотв. ti.

Терм B = AO - пример терма А.

C = AO1, C = BO2 => С - общий пример термов А и В.

Процедурные и декларативные особенности пролога:

Процедура - объединенная группа знаний. Совокупность предложений, описывающих одно знание. Совокупность правил, заголовки которых имеют одинаковый главный функтор, одинаковое количество аргументов, обозначающих объекты одной природы. Процедура - это одно знание, но оно может быть зафиксировано с помощью нескольких предложений. Эта процедура описывается в разделе предикат.

### Механизм логчиеского вывода.  Простейшие правила вывода.

Правило вывода - утверждение о взаимосвязи между допущениями и заключениями (выводами), которые с позиции исчисления предикатов верны всегда.

Всего может быть 4 варианта:

* Факты и вопрос не содержат переменных
* Факт содержит переменные, вопрос нет (Правило конкретизации факта)
* Факт не содержит переменные, вопрос содержит (Правило обощения факта)
* И факт, и вопрос содержат переменные

Доказательство: алгоритм унификации. Система сравнивает формулировку вопроса и формулировку знания (заголовок). Позиционное сравнение составных термов.

Доказательство состоит из 2 шагов: конкретизация правила + обобщение факта.

### Унификация тела.

Операция , которая позволяет формализовать процесс логического вывода. Позволяет понять, про одно и то же ли знания. Можно ли построить общий пример? Основной вычислительный шаг, с помощью которого происходит:

* Двунаправленная передача параметров процедурам
* Неразрушающее присваивание (конкретизация)
* Проверка условий (сравнить T1 = T2)
+ T1 = T2 = const, только если они совпадают, то идентифицируются
+ T1 не конкретизирована (не имеет значения), а T2 = const или составной терм, не содержащий T1, то унификация успешна, а T1 конкретизируется значением T2.
+ T1 и T2 не конкретизированы, то их унификация всегда успешна, причем T1 и T2 становятся сцепленными, т.е. два имени одного и того же объекта.
+ T1 и T2 составные термы, то они успешно унифицируются, если:
	1) у T1 и T2 одинаковые главные функторы
	2) имеют равные арности
	3) успешно унифицируется каждая позиционная пара составных компонентов
